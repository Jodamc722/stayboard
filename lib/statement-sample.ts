...
//
//   rental 2,456.00 + RM reimbursement 2.90 + cleaning channel fee 54.00 + parking 75.00
//     = property income 2,587.90
//   - management 491.20 (20% of rental) - supplies 86.80 - maintenance 168.00 (4.2h @ $40)
//     = ending balance 1,841.90, which is the payment due.
//
...
//
// TWO CHANNEL LINES, NOT ONE, AND THEY ARE NOT THE SAME SIZE (Jon, 2026-09-17: "rm fee
// reimbursement is different than cleaning reimbursement fee"). This sample carried only the RM
// one and I had started to fold a 15%-of-cleaning rule into it, which would have put one wrong
// number where two right ones belong. Counted across our own owner ledger:
//
//   Airbnb RM channel fee reimbursement   10,994 rows, average  1.69, up to  179.53
//   Airbnb cleaning channel fee            9,968 rows, average 21.46, up to   59.29
//
// Both are credits to the owner. Sign is read against Net Rental Nightly Income, which averages
// -134.09 across 89,996 of its 90,204 rows, while Commission averages +19.04 — so in that ledger
// a negative is money to the owner, and both channel lines are negative. The cleaning one is 15%
// of the cleaning fee, which on the $180 clean in this example is $27.00 a booking; the book's
// own 21.46 average implies a typical clean nearer $143, the same rule on smaller units.
//
// The row labels below are the ledger's own names, so an owner comparing this sample to the
// statement Guesty issues them is reading the same words.
//
...
//
// So the detail foots to the booking subtotal -- 849.10 + 1,247.60 = 2,096.70 -- and the two
// expense lines carry that down to the 1,841.90 payout. Twenty-one
...
export const SAMPLE_STATEMENT: SampleStatement = {
  ...
  summary: [
    ...
    { k: 'Airbnb RM channel fee reimbursement', v: m(2.9) },
    { k: 'Airbnb cleaning channel fee', v: m(54) },
    { k: 'Parking', v: m(75) },
    ...
    { k: 'Ending balance', v: m(1841.9), rule: true },
  ],
  due: { k: 'Payment due to owner', v: m(1841.9) },
  reservations: [
    {
      ...
      lines: [
        ...
        { date: 'Aug 2', desc: 'Airbnb RM channel fee reimbursement', cat: 'Airbnb RM channel fee reimbursement', amt: m(2.9) },
        { date: 'Aug 2', desc: 'Airbnb cleaning channel fee \u2014 15% of the cleaning fee', cat: 'Airbnb cleaning channel fee', amt: m(27) },
      ],
      total: m(849.1),
    },
    {
      ...
      lines: [
        ...
        { date: 'Aug 14', desc: 'Airbnb cleaning channel fee \u2014 15% of the cleaning fee', cat: 'Airbnb cleaning channel fee', amt: m(27) },
      ],
      total: m(1247.6),
    },
  ],
  propertyIncome: { k: 'Property income', v: m(2587.9) },
}
