...
export const CORE_TOOLS: EveTool[] = [
...

  {
    ...
    run: async (input, ctx) => {
      ...
      // NO CALENDAR BLOCKS FROM EVE, NOT EVEN WITH A YES (Jon, 2026-09-23: "make sure calendar blocks are
      // not possible"). A block takes a night off sale and moves a clean; a person does that on the
      // Schedule page. She can say which day she would block and why.
      if (action === 'calendar_block') return { ok: false, error: 'You do not block or unblock calendars, not even with approval. Tell them which unit and day you would block and why, and that a person does it on the Schedule page.' }
