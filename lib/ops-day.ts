// TODAY IN OPS — the day builder (extracted from app/api/ops-today/route.ts on 2026-09-02 so the
// Command Center engine can read the SAME day the board reads, in-process, instead of a second
// copy of these rules). The route is now a thin wrapper around buildOpsDay().
// live operational picture, organised BY UNIT (a unit can have a strip, a
...
export async function buildOpsDay(dateParam: string | null, opts: { includeMeta?: boolean } = {}) {
  ...
  for (const t of tasks) {
    if (!unitMap[t.listingId]) {
      unitMap[t.listingId] = {
        ...
        bedrooms: (lmap[t.listingId] && (lmap[t.listingId] as any).bedrooms) ?? null,
        building: (lmap[t.listingId] && (lmap[t.listingId] as any).building) || null,
  ...
  // recompute arrivals/departures/same-day turns from its own queries and disagreed with this
  // board on the same screen: 2026-09-02 it said "0 same-day turns" beside three SAME-DAY rows).
  const activeIds = Object.keys(lmap).filter(id => lmap[id].active)
  const pulse = {
    active: activeIds.length,
    occupiedTonight: activeIds.filter(id => !!occupied[id]).length,
    arrivals: Object.keys(inToday).length,
    departures: Object.keys(outToday).length,
    sameDayTurns: Object.keys(outToday).filter(id => inToday[id]).length,
    vacant: vacants.length,
  }
