...
import { getGuestOrdersCfg, loadCatalog, orderByFor, timingFor, hubOf, fmtDay, fmtTimeET, todayET, addDays, listStock, sizeLabel } from '@/lib/guest-orders'
...
export async function GET(req: NextRequest) {
  ...
  const data = {
    ...
    catalog: catalog.map(c => ({ sku: c.sku, name: c.name, description: c.description, price: c.price_usd, salePrice: c.sale_price_usd, badge: c.badge, soldIn: c.sold_in, pieces: c.pieces, pieceName: c.piece_name, unit: c.unit_label, size: sizeLabel(c), category: c.category || 'Extras', maxQty: c.track_stock && c.available != null ? Math.min(c.max_qty, c.available) : c.max_qty, tiers: c.tiers || null, image: c.image_url, fewLeft: c.track_stock && c.available != null && c.available <= 3 ? c.available : null, id: c.id })),
    orders: [],
  }
