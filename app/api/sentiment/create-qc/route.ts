...
export async function POST(req: NextRequest) {
  ...
  const payload: Record<string, any> = { name: title, type_department: department, type_priority: priority, scheduled_date: date || todayET(), description }
  ...
  try { await db.from('qc_tasks').insert({ breezeway_task_id: taskId, listing_id: listingId, conversation_id: conversationId || null, issue_type: issueType, guest_name: guestName, title, description, department, report_url: reportUrl, status: 'open', created_by: user.email || null }) } catch { /* log optional */ }
