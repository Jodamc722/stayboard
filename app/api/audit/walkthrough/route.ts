...
export async function POST(req: NextRequest) {
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }, body: JSON.stringify({ model: 'claude-opus-4-8', max_tokens: 1500, system: SYS, messages: [{ role: 'user', content: 'Rooms in this unit: ' + (rooms.join(', ') || 'unknown') + '. Dictation: ' + transcript }] }) })
    ...
    const items = (parsed && Array.isArray(parsed.items) ? parsed.items : []).slice(0, 30).map((x: any) => ({ room: String(x.room || 'General').slice(0, 80), kind: KINDS.includes(String(x.kind)) ? String(x.kind) : 'maintenance', title: String(x.title || '').slice(0, 160), note: String(x.note || '').slice(0, 400) })).filter((x: any) => x.title)
