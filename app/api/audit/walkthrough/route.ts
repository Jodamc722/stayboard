...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const items = (parsed && Array.isArray(parsed.items) ? parsed.items : []).slice(0, 30).map((x: any) => {
      ...
      // Smart clarifier: "What size TV?|43 inch|55 inch" -> {q, opts[]} chips in the review UI.
      const askParts = String(x.ask || '').slice(0, 220).split('|').map((s: string) => s.trim()).filter(Boolean)
      const ask = askParts.length >= 2 ? { q: askParts[0].slice(0, 90), opts: askParts.slice(1, 5).map((s: string) => s.slice(0, 40)) } : null
      return { room: String(x.room || 'General').slice(0, 80), kind: KINDS.includes(String(x.kind)) ? String(x.kind) : KINDS[0], title: String(x.title || '').slice(0, 160), note: String(x.note || '').slice(0, 400), photo: String(x.photo || '').slice(0, 120), tags, ask }
