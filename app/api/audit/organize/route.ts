...
async function visionOne(key: string, content: any[], room: string): Promise<{ items: any[]; questions: string[] } | null> {
  const SYS = 'You are doing a QUICK, smart onboarding inventory of a short-term rental ' + room + ' from SEVERAL photos of the SAME room. Look at EVERY photo together and REASON before you speak. READ CAREFULLY: examine logos, badges, brand marks and model numbers on TVs, kitchen appliances, thermostats and electronics - close-up and logo shots exist SO THAT you can read the brand and model, so report EXACTLY what the logo says (e.g. LG, Samsung, Nespresso, Keurig, Bosch). Cross-reference: if one photo shows the device and another shows its logo, remote or a close-up, combine them to name the exact brand and model. RULES: there is normally only ONE of each big appliance in a room (one TV, one thermostat). A close-up, a brand or logo shot, or a REMOTE is NOT a separate item - it is more evidence about that SAME device, so MERGE it in and NEVER create a second TV. A remote reveals its device: an LG remote means the TV is an LG - use it to fill the brand and model. Capture the MAJOR furniture and the functional inventory a guest uses: bed, nightstands, TV, seating, dresser, desk, and real appliances and equipment (in a kitchen the knife set, utensils, cookware, blender, toaster, coffee maker, kettle, microwave, dishwasher, oven). Do NOT list decor, art, plants, trinkets, curtains, rugs, mirrors, doors, windows, or architectural features - this is an OPERATIONS onboarding, not a design catalog. ALWAYS capture operations items when visible: breaker panel, water shut-off valve, water heater, smoke or CO detector, and AC or furnace filter - read the filter SIZE off its label and put it in size. Give a COUNT per kind (two nightstands = count 2, not two rows). Put the KEY attribute in size: bed -> King/Queen/Full/Twin; TV -> Smart or Standard (always include brand if any photo shows it); bathroom -> Shower, Tub, or Shower + Tub; closet -> Walk-in or Reach-in. BRANDS MATTER: for every appliance and tech item - especially kitchen appliances (coffee maker, blender, toaster, microwave, oven, dishwasher), the thermostat, TVs, and washer/dryer - ALWAYS record the brand (and model if any photo shows it) and ALWAYS write a clear step-by-step howTo manual a guest can follow to operate it (power on, key buttons/settings, what to avoid). These manuals feed the FAQ, so write one even if it seems obvious. Leave howTo empty only for plain non-mechanical items like a knife set, utensils, cookware, or furniture. For each item set photoIndex to the photo number that best shows it. QUESTIONS: think hard and ask ONLY what you truly cannot see or infer from ANY photo. If a close-up, logo, or remote reveals the brand, model, or size, do NOT ask about it. Never ask whether something exists that you can see, and never ask the same thing twice. Prefer ZERO questions; max 2. Write each question as the question text followed by 2-4 short answer options separated by pipe characters, like this: What size is the bed?|King|Queen|Full. STRICT JSON ONLY, no markdown: {"items":[{"item":"","itemType":"","count":1,"size":"","brand":"","tier":"luxury|high_end|standard|budget|unknown","condition":"","severity":"low|medium|high","amenity":true,"highlight":true,"howTo":"","photoIndex":1,"area":"matching feature tag or empty"}],"questions":["..."]}'
  try {
    const ac = new AbortController(); const timer = setTimeout(() => ac.abort(), 35000)
    const doCall = (model: string) => aiFetch('audit', { method: 'POST', headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'Content-Type': 'application/json' }, signal: ac.signal, body: JSON.stringify({ model, max_tokens: 2000, system: SYS, messages: [{ role: 'user', content }] }) })
    let r: any = null
...
export async function POST(req: NextRequest) {
  ...
  const tags: string[] = Array.isArray(body.tags) ? body.tags.slice(0, 20).map((x: any) => String(x || '').slice(0, 40)).filter(Boolean) : []
  const hintsIn: any[] = Array.isArray(body.hints) ? body.hints.slice(0, 12) : []
  const hintFor = (u: string): string => { for (const h of hintsIn) if (h && h.photo === u && h.label) return String(h.label).slice(0, 60); return '' }
  ...
  const batchOut = await Promise.all(batches.map(async (batch) => {
    ...
    batch.forEach((x, i) => { const hh = hintFor(x.u); content.push({ type: 'text', text: 'Photo ' + (i + 1) + (hh ? ' - operator labeled this shot: ' + hh : '') + ':' }); content.push(x.img) })
    content.push({ type: 'text', text: 'Now give the inventory.' + (tags.length ? ' Operator tagged this room features (ground truth - use for bed size, ensuite, walk-in vs reach-in closet, etc, and set each item area to the matching tag): ' + tags.join(', ') + '.' : '') + (answers ? ' Inspector already told you (use this, do not re-ask): ' + answers : '') })
