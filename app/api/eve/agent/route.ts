...
import {
  ACTIONS, RUNG_MEANING, getAgentSettings, saveAgentSettings, agentToday, agentLog, listProposals,
  executeProposal, rejectProposal,
...
export async function GET(req: NextRequest) {
  ...
  if (sp.get('queue')) {
    return NextResponse.json({ ok: true, queue: await listProposals(60) })
  }
  const [settings, today] = await Promise.all([getAgentSettings(), agentToday()])
  return NextResponse.json({ ok: true, settings, today, actions: ACTIONS, rungs: RUNG_MEANING })
}
