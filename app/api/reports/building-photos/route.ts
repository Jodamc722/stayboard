...
import { buildingPhotos, buildingPhotoPools, buildingListingPhotos } from '@/lib/building-photos'
...
export async function GET(req: NextRequest) {
  ...
  if (req.nextUrl.searchParams.get('all') === '1') {
    const [pools, listings] = await Promise.all([buildingPhotoPools(24), buildingListingPhotos(12)])
    const photos: Record<string, string> = {}
    for (const b of Object.keys(pools)) if (pools[b][0]) photos[b] = pools[b][0]
    return NextResponse.json({ ok: true, photos, pools, listings })
  }
