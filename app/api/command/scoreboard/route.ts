...
//
// /api/command/day is THE DAY. This is THE WEEK: everything week-to-date in Eastern time, with a
// "vs last week" delta measured against the SAME weekdays of last week (Monday-to-Thursday against
// Monday-to-Thursday, never a part-week against a whole one). The week starts on the labor
// setting `week_start` (labor_settings, then app_settings), else Monday.
//
...
// `degraded` with the reason instead of a number. Cached five minutes (tag 'scoreboard'). Money
// is redacted per viewer AFTER the cache, so the cached copy is the same for everyone.
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ ...board, tiles: board.tiles.map(t => showMoney ? (({ money: _m, ...rest }) => rest)(t) : redact(t)) })
  } catch (e: any) {
