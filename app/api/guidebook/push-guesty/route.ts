...
export async function POST(req: NextRequest) {
  ...
  const strVals = (f: any) => { try { return Object.values(f || {}).filter((x) => typeof x === 'string') as string[] } catch { return [] } }
  const matchGB = (f: any) => isGuidebook(nameOf(f)) || strVals(f).some((s) => isGuidebook(s))
  let fieldId = ''
  ...
  if (!fieldId) {
    ...
    for (const u of urls) {
      try {
        ...
        for (const f of arr) { const nmv = nameOf(f) || (strVals(f)[0] || ''); if (nmv) available.push(nmv) }
        const gf = arr.find((f: any) => matchGB(f))
