...
export async function POST(req: NextRequest) {
  ...
  const photoAssign: Record<string, string | null> = {
    cover: take(usable.filter(p => p.coverWorthy && SCENE.includes(p.category)).sort((a, b) => b.quality - a.quality)[0]) || pick(['living', 'bedroom', 'view', 'exterior'], true),
