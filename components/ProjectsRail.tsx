...
import { Home, ListChecks, KanbanSquare, Plus, Lock, Search, ChevronDown, ChevronRight, Repeat } from 'lucide-react'
...
export function ProjectsRail() {
  ...
  return (
    <>
      {/* Desktop: a rail. */}
      <aside className="hidden lg:block w-[240px] shrink-0 sticky top-2 max-h-[calc(100vh-1rem)] overflow-y-auto rounded-2xl border border-line bg-app/70 p-2.5">
        {body}
      </aside>
      ...
      <div className="lg:hidden mb-3">
        <div className="flex items-center gap-2">
          <button onClick={() => setMobileOpen(o => !o)}
            className="flex-1 flex items-center gap-2 rounded-xl border border-line bg-white px-3 py-2 text-[13px] font-semibold text-ink text-left">
        ...
        {mobileOpen && <div className="mt-2 rounded-2xl border border-line bg-app/70 p-2.5">{body}</div>}
      </div>
    </>
  )
}
