...
// signature images + PDF record). Saved to app_settings via /api/settings/salato-verify-email.
export function SalatoVerifyEmailAdmin() {
  ...
  useEffect(() => {
    fetch('/api/settings/salato-verify-email', { cache: 'no-store' })
      .then(r => r.json())
      .then(j => { if (j && j.ok) { setEmails(j.emails || ''); setEnabled(j.enabled !== false); setValid(j.valid || []) } else setMsg(j?.error || 'Could not load') })
  ...
  const save = async () => {
    setBusy(true); setMsg('')
    try {
      const r = await fetch('/api/settings/salato-verify-email', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ emails, enabled }) })
      const j = await r.json()
      if (!r.ok || !j.ok) { setMsg(j?.error || 'Could not save'); setBusy(false); return }
      setValid(j.valid || []); setMsg('Saved' + (j.valid && j.valid.length ? ' — ' + j.valid.length + ' recipient' + (j.valid.length === 1 ? '' : 's') : ''))
