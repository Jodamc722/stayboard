...
import { Brain, Mic, Compass, Trash2, Plus, Save, X, Check, TrendingUp, Pencil, Zap, KeyRound, Hash, MapPin, RefreshCw, ShieldAlert, BellOff, Clock, HelpCircle, Layers, Merge } from 'lucide-react'
...
export function EveAdmin({ canEdit }: { canEdit: boolean }) {
  const [tab, setTab] = useState<'memory' | 'voice' | 'direction' | 'approvals' | 'audits'>('memory')
  return (
    <div>
      <div className="flex items-center gap-1 mb-3 border-b border-line">
        {([['memory', 'Memory', Brain], ['voice', 'Voice', Mic], ['direction', 'Direction', Compass], ['approvals', 'Approvals', KeyRound], ['audits', 'Audits', ShieldAlert]] as const).map(([k, label, Icon]) => (
