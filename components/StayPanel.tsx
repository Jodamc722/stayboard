      ...
      {open && (
        <div className="border-t border-line grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-px bg-line/60">
          ...
          {show('stay') && <Box icon={<DollarSign size={12} />} title="Stay & money">
            ...
            {s.money && <Line>Total {money(s.money.total)} · paid {money(s.money.paid)}{s.money.balance ? (s.flags.some((x: any) => x.key === 'balance') ? <> · <span className="text-amber-700 font-semibold">owes {money(s.money.balance)}</span></> : <span className="text-muted"> · {money(s.money.balance)} to come from {s.channel}</span>) : ''}{s.money.refunded ? <> · <span className="text-rose-700">refunded {money(s.money.refunded)}</span></> : ''}</Line>}
