...
type Clean = { listingId: string; unit: string; market: string; hub: string; date: string; guestOut: string | null; nights: number | null; bedrooms: number | null; checkInTime: string | null; checkOutTime: string | null; sameDayTurn: boolean; nextArrival: string | null; doorCode: string | null; newDoorCode: string | null; cleaningTime?: string | null; vendor?: string | null; assignedIds?: number[]; assignedNames?: string[] ; syncStatus?: string; breezewayTaskId?: string | null; manual?: boolean; bzOnly?: boolean; taskDate?: string | null; blocked?: boolean; blockedFrom?: string | null; blockedUntil?: string | null }
                  ...
                  {rows.map((c, i) => {
                    ...
                    return (
                      <tr key={keyOf(c)} className={`border-t ${newBuilding ? 'border-line/80 border-t-2' : 'border-line'} ${blockStaged[keyOf(c)] ? 'bg-red-100' : selected[keyOf(c)] ? 'bg-brand-50/40' : c.sameDayTurn ? 'bg-rose-50/40' : ''}`}>
                        <td className="px-2 py-1.5 align-middle"><input type="checkbox" checked={!!selected[keyOf(c)]} onChange={ev => toggleSelect(c, ev.target.checked)} className="accent-brand-600" />{c.syncStatus === 'guesty-only' && <span title="In Guesty but not synced to Breezeway yet"><AlertTriangle size={12} className="inline text-amber-500 ml-0.5" /></span>}{c.taskDate && <span title={'Breezeway has this clean scheduled on ' + c.taskDate + ' (moved off the checkout day)'} className="inline-block ml-0.5 text-[9px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded px-1 align-middle">&rarr;{c.taskDate.slice(5)}</span>}</td>
