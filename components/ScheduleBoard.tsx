...
const [sugAdded, setSugAdded] = useState<Record<string, string | null>>({})
  ...
  useEffect(() => { const _sp = new URLSearchParams(window.location.search); const _d = _sp.get('date') || new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York' }).format(new Date()); setView('day'); setDate(_d); load('day', _d) }, [])
  ...
  function statusDot(c: Clean): { sym: string; cls: string; label: string } {
    const st = String(c.taskStatus || '')
    if (st === 'completed') return { sym: '✓', cls: 'text-emerald-600', label: 'Completed' }
    if (st === 'in_progress') return { sym: '●', cls: 'text-emerald-500', label: 'In progress' }
    ...
    if (isToday && !c.vendor) {
      ...
      if (h >= 15) return { sym: '●', cls: 'text-red-500', label: 'Not started - late' }
      if (h >= 14) return { sym: '●', cls: 'text-amber-500', label: 'Not started' }
    }
    return { sym: '○', cls: 'text-neutral-300', label: 'Not started' }
  }
