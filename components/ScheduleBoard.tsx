              ...
              <table className="w-full text-[12px] border-collapse">
                ...
                <tbody>
                  {rows.map((c, i) => {
                    ...
                    return (
                      <tr key={keyOf(c)} className={`group border-t ${newBuilding ? 'border-line/80 border-t-2' : 'border-line'} ${blockStaged[keyOf(c)] ? 'bg-red-100' : selected[keyOf(c)] ? 'bg-brand-50/40' : c.taskStatus === 'completed' ? 'bg-emerald-50/70' : c.taskStatus === 'in_progress' ? 'bg-yellow-50' : c.sameDayTurn ? 'bg-rose-50/40' : ''}`}>
