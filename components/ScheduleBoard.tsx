...
// Roster roles, staffing ratio and the growth buffer are operator-editable (/users -> Ops presets);
// these components read them via useOpsPresets() and fall back to today's values.
...
const HUB_COLOR = (hub: string) => {
  ...
  const palette = ['bg-slate-100 text-slate-600', 'bg-sky-50 text-sky-700', 'bg-emerald-50 text-emerald-700', 'bg-amber-50 text-amber-700', 'bg-violet-50 text-violet-700', 'bg-rose-50 text-rose-600', 'bg-teal-50 text-teal-700', 'bg-indigo-50 text-indigo-700']
...
// VISIBILITY ONLY — this is task duration from the Breezeway timer, NOT labor cost.
function cleanTimeCls(bedrooms: number | null | undefined, mins: number, timing: Timing = DEFAULT_TIMING): string { const max = benchmarkMinutes(timing, bedrooms); return mins > max ? 'text-amber-700 bg-amber-50 border-amber-200' : 'text-emerald-700 bg-emerald-50 border-emerald-200' }
...
export function ScheduleBoard() {
  ...
  const DEFAULT_RATE = presets.roster.rate
  const GROWTH = presets.roster.growth
...
// Comment thread per clean (keyed by the Breezeway task id) + unread-free counts for the chip.
...
async function pushOne(c: Clean) {
    if (!data?.breezeway) return
    const k = keyOf(c)
    const e = effective(c)
    setTaskAct(prev => ({ ...prev, [k]: true }))
    try {
      const r = await fetch('/api/schedule/assign', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: [{ listingId: c.listingId, date: c.date, assigneeIds: e.ids, description: descFor(c), taskId: c.breezewayTaskId || null, sameDayTurn: c.sameDayTurn }] }) })
      const raw = await r.text(); let j: any = null; try { j = raw ? JSON.parse(raw) : null } catch { j = null }
      if (!r.ok || !j) throw new Error((j && j.error) || 'Push failed.')
      if (j.pushed) { setSelected(prev => { const n = { ...prev }; delete n[k]; return n }); setPushMsg('Pushed ' + c.unit + ' to Breezeway.') }
      else throw new Error((j.results && j.results[0] && j.results[0].error) || 'Could not resolve the clean yet.')
                ...
                <tbody>
                  {rows.map((c, i) => {
                    ...
                    return (
                      ...
                      <tr className={`group hover:brightness-95 transition border-t ${newBuilding ? 'border-line/80 border-t-2' : 'border-line'} ${blockStaged[keyOf(c)] ? 'bg-red-100' : selected[keyOf(c)] ? 'bg-brand-50/40' : c.taskStatus === 'completed' ? 'bg-emerald-50/40' : (!e.label && !overrides[keyOf(c)] && !c.vendor) ? 'bg-amber-50/40' : ''}`}>
                        <td className="px-2.5 py-1.5 align-middle"><input type="checkbox" checked={!!selected[keyOf(c)]} onChange={ev => toggleSelect(c, ev.target.checked)} className="accent-brand-600" />{c.syncStatus === 'guesty-only' && !c.guestyOnly && <span title="In Guesty but not synced to Breezeway yet"><AlertTriangle size={12} className="inline text-amber-500 ml-0.5" /></span>}</td>
                        ...
                        <td className="px-2.5 py-1.5 align-middle font-medium text-ink">{c.vendor && <span className="mr-1 text-[9px] font-semibold px-1 py-0.5 rounded bg-amber-50 text-amber-700 align-middle">vendor</span>}{statusRing(c)}{c.listingId ? <button type="button" onClick={() => setOpsFor({ listingId: String(c.listingId), unit: String(c.unit), date })} className="text-left font-semibold whitespace-nowrap hover:underline decoration-dotted underline-offset-2">{c.unit}</button> : c.unit}{c.sameDayTurn && <span className="ml-1 inline-flex items-center gap-0.5 text-[10px] font-semibold text-rose-600"><Repeat size={9} /> Same-day turn</span>}{c.movedTo && <span title={'Moved - Breezeway now has this clean scheduled on ' + c.movedTo} className="inline-block ml-0.5 text-[9px] font-bold text-amber-800 bg-amber-50 border border-amber-300 rounded px-1 align-middle">Moved &rarr; {c.movedTo.slice(5)}{(() => { const dd = Math.round((+new Date(c.movedTo + 'T12:00:00') - +new Date(c.date + 'T12:00:00')) / 86400000); return dd > 0 ? ' (+' + dd + 'd)' : '' })()}</span>}
                        ...
                        {(c as any).calNote === 'blocked' && <span title="Guesty calendar shows a BLOCK (owner hold / maintenance) covering the gap - this move is a calendar block, not a guest extension" className="inline-block ml-0.5 text-[9px] font-bold text-slate-700 bg-slate-100 border border-slate-300 rounded px-1 align-middle">Unit blocked</span>}
                        {(c as any).calNote === 'booked' && <span title="Guesty calendar shows a reservation covering the gap - the guest is still in the unit (extension confirmed by calendar)" className="inline-block ml-0.5 text-[9px] font-bold text-violet-700 bg-violet-50 border border-violet-300 rounded px-1 align-middle">Cal: occupied</span>}
                        ...
                        <td className="px-2.5 py-1.5 align-middle text-ink/90 whitespace-nowrap">{c.walkInRisk && <span className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-rose-600 rounded px-1.5 py-0.5 mr-1" title="A confirmed guest is still in-house this day - do NOT clean / walk-in risk">⚠ Guest in-house</span>}{c.missing && <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-700 bg-rose-50 border border-rose-300 rounded px-1.5 py-0.5 mr-1" title="No Breezeway clean scheduled for this checkout within 14 days">⚠ No clean scheduled</span>}{c.manual ? <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-sky-700 bg-sky-50 border border-sky-200 rounded px-1.5 py-0.5">Added manually</span> : c.bzOnly ? (c.breezewayReportUrl ? <a href={c.breezewayReportUrl} target="_blank" rel="noreferrer" title="MOVED-IN CLEAN - scheduled in Breezeway on this day with no checkout here. Click to open the task in Breezeway and check it." className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-1.5 py-0.5 hover:bg-emerald-100 underline decoration-dotted">Moved-in clean &rarr;</a> : <span title="MOVED-IN CLEAN - scheduled in Breezeway on this day with no checkout here" className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-1.5 py-0.5">Moved-in clean</span>) : (c.guestOut ? <span className="inline-block max-w-[150px] truncate align-bottom" title={c.guestOut}>{c.guestOut}</span> : <span className="text-muted italic">—</span>)}</td>
                        
                        <td className="px-2.5 py-1.5 align-middle text-right"><span className="inline-flex items-center justify-end gap-1.5 opacity-80 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-150">{data.breezeway && <button onClick={() => c.blocked ? unblockClean(c) : toggleBlockStage(c)} disabled={!!blocking[keyOf(c)]} className={'align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border disabled:opacity-40 ' + (c.blocked ? 'text-amber-700 border-amber-200 bg-amber-50 hover:bg-amber-100' : blockStaged[keyOf(c)] ? 'text-red-600 border-red-200 bg-red-50 font-bold' : 'text-muted border-line bg-white hover:text-amber-700 hover:bg-amber-50')} title={c.blocked ? 'Move back to the original day' : (blockStaged[keyOf(c)] ? 'Staged - click to unstage' : 'Not ready today - stage to move to tomorrow')}>{c.blocked ? 'Undo move' : (blockStaged[keyOf(c)] ? 'Staged' : 'Move')}</button>}{c.guestyOnly && <span title="This building is not in Breezeway - the checkout comes from Guesty and the vendor cleans it. No task to push or track." className="align-middle text-[10px] font-semibold uppercase px-1.5 py-1 rounded-md border border-slate-300 bg-slate-100 text-slate-600">Guesty only</span>}{data.breezeway && !c.guestyOnly && c.syncStatus === 'guesty-only' && <button onClick={() => pushMissingClean(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100 disabled:opacity-40" title="Not in Breezeway - create it there (only on your click, never automatic)">Push</button>}{data.breezeway && c.syncStatus === 'synced' && c.breezewayTaskId && <button onClick={() => pushOne(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100 disabled:opacity-40" title="Push this clean to Breezeway now (just this row)">Push</button>}{data.breezeway && c.syncStatus === 'synced' && c.breezewayTaskId && <button onClick={() => deleteClean(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-line bg-white text-muted hover:text-rose-700 hover:border-rose-300 disabled:opacity-40" title="Delete this clean from Breezeway">Delete</button>}{data.breezeway && c.breezewayReportUrl && <a href={c.breezewayReportUrl} target="_blank" rel="noreferrer" className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100" title="VIEW the field report (read-only)">Report</a>}{data.breezeway && c.breezewayTaskId && <a href={'https://app.breezeway.io/task/' + c.breezewayTaskId} target="_blank" rel="noreferrer" className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100" title="Open the ADMIN task in Breezeway — edit, assign, modify, check">Admin</a>}{data.breezeway && c.breezewayTaskId && <button onClick={() => addNote(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border border-line bg-white text-muted hover:text-ink hover:bg-app disabled:opacity-40" title="Add a note onto the Breezeway task (housekeeping sees it in the task description)">Note</button>}{data.breezeway && c.breezewayTaskId && <button onClick={() => setCmtFor(cmtFor === c.breezewayTaskId ? '' : String(c.breezewayTaskId))} className={'align-middle text-[11px] font-semibold px-1.5 py-1 rounded-md border inline-flex items-center gap-1 ' + (cmtFor === c.breezewayTaskId ? 'bg-ink text-white border-ink' : cmtCounts[String(c.breezewayTaskId)] ? 'border-sky-300 bg-sky-50 text-sky-700' : 'border-line bg-white text-muted hover:text-ink hover:bg-app')} title="Comment on this clean - teammates you tag get a notification, and everyone on the thread hears about replies"><MessageSquare size={11} />{cmtCounts[String(c.breezewayTaskId)] || ''}</button>}</span></td>
                      </tr>
                      {c.breezewayTaskId && cmtFor === String(c.breezewayTaskId) && (
                        <tr className="border-t border-line bg-app/40">
                          <td colSpan={99} className="px-2.5 py-2">
                            <CommentThread type="task" id={String(c.breezewayTaskId)} label={c.unit + ' — clean ' + c.date} link="/schedule" taskId={String(c.breezewayTaskId)} onCount={n => setCmtCounts(prev => ({ ...prev, [String(c.breezewayTaskId)]: n }))} />
                          </td>
                        </tr>
                      )}
                      </Fragment>
