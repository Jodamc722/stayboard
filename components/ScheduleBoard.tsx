              ...
              <table className="w-full text-[12px] border-collapse">
                <thead>
                  <tr className="bg-neutral-50 text-neutral-500 text-[10px] uppercase tracking-wider text-left font-semibold border-b border-line">
                    ...
                    <th className="px-2 py-2 font-semibold sticky right-0 bg-neutral-50 z-10"></th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((c, i) => {
                    ...
                    return (
                      <tr key={keyOf(c)} className={`group hover:brightness-95 transition border-t ${newBuilding ? 'border-line/80 border-t-2' : 'border-line'} ${blockStaged[keyOf(c)] ? 'bg-red-100' : selected[keyOf(c)] ? 'bg-brand-50/40' : ''}`}>
                        ...
                        <td className="px-2.5 py-1.5 align-middle text-right sticky right-0 bg-white z-[1] border-l border-line"><span className="inline-flex items-center justify-end gap-2.5 opacity-70 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-150">{data.breezeway && <button onClick={() => c.blocked ? unblockClean(c) : toggleBlockStage(c)} disabled={!!blocking[keyOf(c)]} className={'align-middle text-[10px] font-semibold disabled:opacity-40 ' + (c.blocked ? 'text-amber-700 hover:text-amber-800' : blockStaged[keyOf(c)] ? 'text-red-600 font-bold' : 'text-muted hover:text-amber-700')} title={c.blocked ? 'Move back to the original day' : (blockStaged[keyOf(c)] ? 'Staged - click to unstage' : 'Not ready today - stage to move to tomorrow')}>{c.blocked ? 'Undo move' : (blockStaged[keyOf(c)] ? 'Staged' : 'Move')}</button>}{data.breezeway && c.syncStatus === 'guesty-only' && <button onClick={() => pushMissingClean(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[10px] font-semibold text-brand-700 hover:text-brand-800 disabled:opacity-40" title="Not in Breezeway - create it there (only on your click, never automatic)">Push</button>}{data.breezeway && c.syncStatus === 'synced' && c.breezewayTaskId && <button onClick={() => pushOne(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[10px] font-semibold text-brand-700 hover:text-brand-800 disabled:opacity-40" title="Push this clean to Breezeway now (just this row)">Push</button>}{data.breezeway && c.syncStatus === 'synced' && c.breezewayTaskId && <button onClick={() => deleteClean(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[10px] font-semibold text-muted hover:text-rose-700 disabled:opacity-40" title="Delete this clean from Breezeway">Delete</button>}{data.breezeway && c.breezewayReportUrl && <a href={c.breezewayReportUrl} target="_blank" rel="noreferrer" className="align-middle text-[10px] font-semibold text-sky-700 hover:text-sky-800" title="Open this clean's task in Breezeway to check it">Task</a>}{data.breezeway && c.breezewayTaskId && <button onClick={() => addNote(c)} disabled={!!taskAct[keyOf(c)]} className="align-middle text-[10px] font-semibold text-muted hover:text-ink disabled:opacity-40" title="Add a note onto the Breezeway task (housekeeping sees it in the task description)">Note</button>}</span></td>
                      </tr>
                    )
                  })}
                </tbody>
