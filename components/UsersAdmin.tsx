...
export function UsersAdmin({ myEmail }: { myEmail: string }) {
  ...
  async function invite(e: React.FormEvent) {
    ...
    try {
      const r = await fetch('/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, role, password: password || undefined }) })
      ...
      setEmail(''); setRole('member'); setPassword(''); load()
      ...
      <form onSubmit={invite} className="rounded-2xl border border-brand-200 bg-white p-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="min-w-[200px]">
            ...
            <input type="text" value={password} onChange={e => setPassword(e.target.value)} placeholder="Set one, or leave blank to email invite" autoComplete="new-password"
              className="w-full text-sm rounded-lg border border-line bg-app px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-200" />
          </div>
        ...
        <p className="text-[11px] text-muted mt-2">Set a password to create their login instantly (share it with them securely) — or leave it blank to email them an invite to set their own. Access is granted immediately either way.</p>
      </form>
