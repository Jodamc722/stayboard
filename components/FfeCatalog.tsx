...
import { Plus, Loader2, Search, Package, ExternalLink, Pencil, Archive, RotateCcw, ClipboardPaste, X } from 'lucide-react'
import { FFE_CATEGORIES, money } from '@/lib/ffe-catalog'
        ...
        <div key={c.key} className="rounded-2xl border border-line bg-white overflow-hidden shadow-soft">
          ...
          <div className="divide-y divide-line">
            {byCategory[c.key].map(p => (
              <div key={p.id} className={'px-4 py-2.5 flex items-center gap-3 ' + (p.active ? '' : 'opacity-50')}>
                {p.image_url
