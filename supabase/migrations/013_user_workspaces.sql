...
-- 2) app_settings: small key/value store for app-wide config edited from the admin console.
--    First key: 'review_voice' = { guidelines, examples: [{review, reply}], updated_by, updated_at }
...
alter table app_settings enable row level security;  -- service-role only (no policies = no anon access)
